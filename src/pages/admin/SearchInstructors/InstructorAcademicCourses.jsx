...
import { useGetInstructorAcademicCourses } from "../../../api/admin/users";


export default function InstructorAcademicCourses() {
  const { id } = useParams();
  const [sortCol, setSortCol] = useState({ key: null, direction: "asc" });

  const { data: courses = [], isLoading, error } = useGetInstructorAcademicCourses(id);
  ...
  if (error) {
    if (error.response?.status === 404) {
      return <div>No academic courses found for this instructor.</div>;
    }
