...
import { useGetInstructorExtraCourses } from "../../../api/admin/users";
...
export default function InstructorAcademicCourses() {
  ...
  const { data: courses = [], isLoading, error } = useGetInstructorExtraCourses(id);
  ...
  if (error) {
    if (error.response?.status === 404) {
      return <div>No extra courses found for this instructor.</div>;
    }
